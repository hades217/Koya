...
"""创建或复用远程技能沙箱会话并通过 A2A 接口发起调用。

本脚本面向的场景：技能沙箱从远程 Skill Space 加载技能集合，由上层 Agent
通过 `execute_skills` 工具将用户请求转发给沙箱内部的 Agent 具体执行技能。

注意：本脚本使用 `skill` profile，CreateSession 时不会注入任何 `MODEL_AGENT_*`
环境变量；模型配置由 Skill Tool 自身负责管理，无需在客户端侧传入。

使用示例：
    python3 advanced/a2a/remote_skills_a2a_invoke.py \
...
SOURCE = "remote-skills-a2a-invoke"
# 本脚本对应技能沙箱场景，使用 skill profile（不注入模型环境变量）
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "创建或复用远程技能沙箱会话（远程 Skill Space + execute_skills）"
            "，并通过 A2A 接口同步发起调用。"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--tool-id",
        required=True,
        help="远程技能沙箱对应的 Sandbox Tool ID。",
    )
    parser.add_argument(
        "--session-id",
        required=True,
    ...
    parser.add_argument(
        "--prompt",
        required=True,
        help="发送给远程技能沙箱 A2A Agent 的用户请求。",
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        # envs=None 表示不注入会话级环境变量，符合 skill profile
        ...
        task_start = send_message_nonblocking(
            ...
            request_metadata={
                ...
                "user_id": "agentkit-remote-skills-sandbox",
            },
