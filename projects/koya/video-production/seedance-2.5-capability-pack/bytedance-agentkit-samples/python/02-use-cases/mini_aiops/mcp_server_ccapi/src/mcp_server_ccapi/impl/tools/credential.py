...
def get_value_from_ctx_or_env(ctx: Context[ServerSession, object], region: str | None) -> dict:
    """Get credential values from the request context or environment variables."""
    credential_info = {}
    # 如果没有配置AK、SK
    if VOLCENGINE_ACCESS_KEY_ENV not in os.environ or VOLCENGINE_SECRET_KEY_ENV not in os.environ:
        ...
        try:
            ...
            credential_info['endpoint'] = (
                f'cloudcontrol.{credential_info["region"]}.volcengineapi.com'
            )
        ...
        credential_info['endpoint'] = (
            os.environ.get(VOLCENGINE_HOST_ENV) or 'cloudcontrol.cn-beijing.volcengineapi.com'
        )
...
class VeIAMCredential(BaseModel):
    """Credential values loaded from the VeFaaS IAM credential file."""
