...
def main() -> None:
    ...
    default_save_object = f"format_{os.path.basename(key)}.{args.fmt}"
    ...
    output_path = None if persist_to_tos else args.output or default_output_path(key, args.fmt)

    plan = {
        "ok": True,
        "operation": "image_format",
        "bucket": bucket,
        "key": key,
    ...
    if persist_to_tos:
        if not args.json:
            print(
                f"[INFO] Converting {bucket}/{key} -> {resolved_save_bucket}/{resolved_save_object}"
            )
