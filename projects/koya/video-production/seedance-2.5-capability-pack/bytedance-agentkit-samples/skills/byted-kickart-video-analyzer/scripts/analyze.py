...
def format_for_origin(data: dict) -> dict:
    ...
    if product_info and isinstance(product_info, dict):
        ...
        product_info_copy["product_description"] = product_info.get(
            "product_description", ""
        )
...
def format_for_seedance(data: dict) -> dict:
    ...
    if role_list and isinstance(role_list, list):
        result["role_list"] = list(
            zip(
                [role.get("id", "") for role in role_list],
                [role.get("vocal_attributes", "") for role in role_list],
            )
        )
        ...
        for _ in range(2 * 5):
            ...
            click.echo(
                Result(
                    code="0",
                    message="success",
                    data={"output": output, "seedance_output": seedance_output},
                ).model_dump_json()
            )
